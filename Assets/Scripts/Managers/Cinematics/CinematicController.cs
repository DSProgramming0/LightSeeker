...
public class CinematicController : MonoBehaviour
{
    ...
    [Header("Player driven Cinemachine")]
    ...
    public void setCurrentPlayerDrivenCinematicComponents(CinematicType _type, CinemachineDollyCart _dollyCart, CinemachineVirtualCamera _vCam, PlayableDirector _director, LightPillarActivation _lightPillar, Transform _playerResetPos, Transform _lookAtPos, float _stopTime)
    ...
    void Update()
    {
        if (PlayerManager.instance.getWorldState() == PlayerWorldState.INCINEMATIC)
        {
            ...
            if(currentCinematicType == CinematicType.PLAYERDRIVEN)
            {
                ...
                if (currentCinematicDirector.time >= currentCinematicDirector.duration - stopTime) //Starts playing beam of light effect before end of animation
                {
    ...
    private IEnumerator playFreeTheLightEffect() //Gives times to play effect / waitForSeconds should be changed depending on lenght of effect
    {
        ...
        {
            BGMusicSelector.instance.changeSong(0);
